...
function errorHandler(id) {
  if (clients.has(id)) {
    ...
    io.emit('sendPlayerOnline',clients.size)
    clients.delete(id)
  }
...
io.on("connection", (socket) => {
  ...
  //players online
  clients.get(socket.id).on('requestPlayerOnline',function() {
    try {
      clients.get(socket.id).emit('sendPlayerOnline',clients.size);
    }
    catch(e) {
      console.log('unable to send # of player online')
    }
  })
